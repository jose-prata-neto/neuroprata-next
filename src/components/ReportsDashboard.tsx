...
const ReportsDashboard: React.FC<ReportsDashboardProps> = ({ patient }) => {
  ...
  const tagFrequencyData = useMemo(() => {
    const tagCounts = filteredSessions.reduce((acc: { [key: string]: number }, session) => {
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          ...
          <input type="date" id="report-startDate" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 sm:text-sm" />
        </div>
        <div>
          <label htmlFor="report-endDate" className="block text-sm font-medium text-slate-700">Data Final</label>
          <input type="date" id="report-endDate" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 sm:text-sm" />
        </div>
      </div>
       ...
       <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {sessionFrequencyData.length > 1 ? (
          ...
          <div className="rounded-lg border-2 border-dashed border-slate-200 p-8 text-center">
            ...
            <h4 className="mt-4 text-lg font-semibold text-slate-700">Dados insuficientes</h4>
            <p className="mt-1 text-slate-500">São necessárias sessões em pelo menos dois meses diferentes para gerar este gráfico.</p>
        </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {tagFrequencyData.length > 0 ? (
          ...
          <div className="rounded-lg border-2 border-dashed border-slate-200 p-8 text-center">
            ...
            <h4 className="mt-4 text-lg font-semibold text-slate-700">Nenhuma tag clínica encontrada</h4>
            <p className="mt-1 text-slate-500">Use a análise de IA ao registrar sessões para gerar tags e visualizar sua frequência aqui.</p>
        </div>
        )}
      </div>
    </div>
