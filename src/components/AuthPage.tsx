    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-100">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg">
        <div className="text-center">
            ...
            <p className="mt-1 text-slate-500">{isLoginView ? 'Bem-vindo(a) de volta!' : 'Crie sua conta'}</p>
        </div>
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          {!isLoginView && (
            <>
              <div>
                <label className="block text-sm font-medium text-slate-700">Tipo de Conta</label>
                <div className="mt-1 grid grid-cols-2 gap-2 rounded-md bg-slate-100 p-1">
                  <button type="button" onClick={() => setRole('psychologist')} className={`px-3 py-1 text-sm font-semibold rounded ${role === 'psychologist' ? 'bg-white shadow text-slate-800' : 'text-slate-600'}`}>Psicólogo(a)</button>
                  <button type="button" onClick={() => setRole('staff')} className={`px-3 py-1 text-sm font-semibold rounded ${role === 'staff' ? 'bg-white shadow text-slate-800' : 'text-slate-600'}`}>Funcionário(a)</button>
                </div>
              </div>
               <div>
          ...
          {!isLoginView && (
             <div>
                <label htmlFor="identifier" className="sr-only">{role === 'psychologist' ? 'CRP' : 'CPF'}</label>
                <input id="identifier" name="identifier" type="text" required value={identifier} onChange={(e) => setIdentifier(e.target.value)}
                  className="relative block w-full appearance-none rounded-md border border-slate-300 px-3 py-2 text-slate-900 placeholder-slate-500 focus:z-10 focus:border-slate-500 focus:outline-none focus:ring-slate-500 sm:text-sm"
                  placeholder={role === 'psychologist' ? 'CRP (ex: 06/123456)' : 'CPF (somente números)'}
                />
              </div>
          )}
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          ...
          <button onClick={toggleView} className="ml-1 font-medium text-slate-600 hover:text-slate-800">
